...
; Noyax top mode background not works on XP
If @OSVersion = "WIN_XP" then GUICtrlSetState($chkBackground, $GUI_DISABLE)
; Noyax bottom
...
;		GUICtrlSetState($txtDBAttMilk, $GUI_DISABLE)
	EndIf
;	GUICtrlSetData($txtchkPercentExposed, $NbPercentExposed)
	GUICtrlSetData($txtchkPixelmaxExposed, $NbPixelmaxExposed)
	GUICtrlSetData($txtDBUseGobsForCollector, $DBUseGobsForCollector)
;	If $PercentExposed = 1 Then
;		GUICtrlSetState($chkPercentExposed, $GUI_CHECKED)
;		GUICtrlSetState($txtDBAttMilk, $GUI_ENABLE)
;	Else
;		GUICtrlSetState($chkPercentExposed, $GUI_UNCHECKED)
;		GUICtrlSetState($txtDBAttMilk, $GUI_DISABLE)
;	EndIf
;Noyax bottom
	;Noyax add Ancient begin archer milking
	GUICtrlSetData($txtDBAttMilkArch, $NbTrpMilkArch)
	GUICtrlSetData($txtDBUseArchForCollector, $DBUseArchForCollector)
	;Noyax add Ancient end archer milking
	;Noyax add Rizor begin Barbarian milking
	GUICtrlSetData($txtDBAttMilkBarb, $NbTrpMilkBarb)
	GUICtrlSetData($txtDBUseBarbForCollector, $DBUseBarbForCollector)
	...
	GUICtrlSetData($txtTSGoblReserve, $iTSGoblReserve)
	GUICtrlSetData($txtTSArchReserve, $iTSArchReserve)
	GUICtrlSetData($txtTSBarbReserve, $iTSBarbReserve)
...
;noyax add Ancient begin greed filter filter with DB
	If $iOptAttIfDBMeetGE = 1 Then
		GUICtrlSetState($chkAttIfDBMeetGE, $GUI_CHECKED)
	Else
