...
;	If GUICtrlRead($chkPercentExposed) = $GUI_CHECKED Then
;		IniWrite($config, "attack", "PercentExposed", 1)
;	Else
;		IniWrite($config, "attack", "PercentExposed", 0)
;	EndIf
;	IniWrite($config, "attack", "NbPercentExposed", GUICtrlRead($txtchkPercentExposed))
	IniWrite($config, "attack", "NbPixelmaxExposed", GUICtrlRead($txtchkPixelmaxExposed))
	IniWrite($config, "attack", "DBUseGobsForCollector", GUICtrlRead($txtDBUseGobsForCollector))
;Noyax bottom
	;noyax add Ancient begin archer milking
	IniWrite($config, "attack", "NbTrpMilkArch", GUICtrlRead($txtDBAttMilkArch))
	IniWrite($config, "attack", "DBUseArchForCollector", GUICtrlRead($txtDBUseArchForCollector))
	;noyax add Ancient end archer milking

	;noyax add Rizor begin Barbarian milking
	IniWrite($config, "attack", "NbTrpMilkBarb", GUICtrlRead($txtDBAttMilkBarb))
	IniWrite($config, "attack", "DBUseBarbForCollector", GUICtrlRead($txtDBUseBarbForCollector))
	;noyax add Rizor end Barbarian milking

	;noyax add Ancient begin snipe live/dead filter
	IniWrite($config, "attack", "TSSearchMode", _GUICtrlComboBox_GetCurSel($cmbTSSearchMode))
	;noyax add Ancient end snipe live/dead filter
	;noyax add Ancient & Rizor begin snipe reserves
	IniWrite($config, "attack", "TSGoblReserve", GUICtrlRead($txtTSGoblReserve))
	IniWrite($config, "attack", "TSArchReserve", GUICtrlRead($txtTSArchReserve))
	IniWrite($config, "attack", "TSBarbReserve", GUICtrlRead($txtTSBarbReserve))
	;noyax add Ancient & Rizor end snipe reserves
...
;noyax add Ancient begin greed filter filter with DB
	If GUICtrlRead($chkAttIfDBMeetGE) = $GUI_CHECKED Then
		IniWrite($config, "advanced", "THsnAttIfDBMeetGE", 1)
	Else
		IniWrite($config, "advanced", "THsnAttIfDBMeetGE", 0)
	EndIf
